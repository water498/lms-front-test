    ...
    <table className="w-full text-sm">
      <thead>
        <tr className="border-b border-slate-200">
          ...
          <th className="text-left px-5 py-3 text-xs font-medium text-slate-400">이메일</th>
          <th className="text-center px-5 py-3 text-xs font-medium text-slate-400">진도</th>
          <th className="text-center px-5 py-3 text-xs font-medium text-slate-400">상태</th>
          <th className="text-left px-5 py-3 text-xs font-medium text-slate-400">등록일</th>
        </tr>
      </thead>
      <tbody>
        {enrollments.map((e) => (
          <tr key={e.userId} className="border-b border-slate-200/50 hover:bg-slate-100/20 transition-colors">
            ...
            <td className="px-5 py-3.5">
              <div className="flex items-center gap-2">
                <div className="flex-1 bg-slate-50 rounded-full h-1.5 max-w-[80px] mx-auto">
                  <div
                    className="bg-violet-500 h-1.5 rounded-full"
                    style={{ width: `${e.progress}%` }}
                  />
                </div>
                <span className="text-xs text-slate-500 w-8 text-right">{e.progress}%</span>
              </div>
            </td>
            ...
            <td className="px-5 py-3.5 text-slate-500">{e.enrolledAt}</td>
          </tr>
        ))}
      </tbody>
    </table>
