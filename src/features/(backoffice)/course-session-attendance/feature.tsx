...
export default function AttendanceTab({ sessionId }: { sessionId: string }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* 선택된 회차 정보 */}
      {selectedOs && (
        <div className="mx-5 bg-slate-50 rounded-xl px-4 py-3 flex items-center gap-6">
          <div>
            <p className="text-xs text-slate-400">회차</p>
            <p className="text-sm font-medium text-slate-900">{selectedOs.title}</p>
          </div>
          ...
          <button
            ...
            className="ml-auto flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-violet-600 border border-slate-200 hover:border-violet-500 rounded-lg transition-colors"
          >
            <QrCode size={13} />
            QR
          </button>
      ...
      {/* 출결 테이블 */}
      {selectedOs?.status === "SCHEDULED" ? (
        <div className="text-center py-8 px-5 text-slate-400">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200">
              ...
              <th className="text-center px-5 py-3 text-xs font-medium text-slate-400">출결 상태</th>
              <th className="text-left px-5 py-3 text-xs font-medium text-slate-400">비고</th>
              <th className="text-left px-5 py-3 text-xs font-medium text-slate-400">체크 시각</th>
            </tr>
          </thead>
          <tbody>
            {enrollments.map((e) => {
              ...
              return (
                <tr key={e.userId} className="border-b border-slate-200/50 hover:bg-slate-100/20 transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-slate-400 text-xs">{att?.note ?? "—"}</td>
...
function InstructorQrModal({ session, onClose }: { session: { id: string; dayNum: number; startsAt: string; status: string }; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-white border border-slate-200 rounded-2xl shadow-xl w-full max-w-sm">
        <div className="flex items-center justify-between px-5 pt-5">
          <h2 className="text-base font-semibold text-slate-900">{session.dayNum}회차 QR 코드</h2>
        ...
        <div className="flex gap-1 mx-5 mt-4 p-1 bg-slate-50 rounded-lg">
          <button onClick={() => setMode("attendance")}
            className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-medium rounded-md transition-colors ${mode === "attendance" ? "bg-slate-200 text-slate-900" : "text-slate-400 hover:text-slate-700"}`}>
            <ClipboardCheck size={13} /> 출결
          </button>
          <button onClick={() => setMode("assessment")}
            className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-medium rounded-md transition-colors ${mode === "assessment" ? "bg-slate-200 text-slate-900" : "text-slate-400 hover:text-slate-700"}`}>
            <FileText size={13} /> 평가
          </button>
        </div>
            ...
            <div className="bg-slate-50 rounded-xl p-4 flex flex-col gap-3 mb-4">
              ...
              <div className="flex items-center gap-2 text-sm text-slate-600">
                ...
                <select value={beforeMin} onChange={(e) => setBeforeMin(Number(e.target.value))}
                  className="bg-slate-50 border border-slate-200 rounded-lg px-2 py-1 text-sm text-slate-900 focus:outline-none focus:border-violet-500">
              ...
              <p className="text-xs text-slate-500">{session.startsAt} 기준 ±{beforeMin}/{afterMin}분</p>
            </div>
          )}

          {mode === "assessment" && selectedActivity && (
            <div className="bg-violet-50 border border-violet-500/20 rounded-xl p-4 mb-4">
              <p className="text-xs text-violet-600">수강생이 QR을 스캔하면 <strong>{selectedActivity.title}</strong> 페이지로 이동합니다.</p>
            </div>
          )}

          <div className="flex items-center gap-2 mb-5">
            <span className={`w-2 h-2 rounded-full ${isActive ? "bg-emerald-500" : "bg-slate-300"}`} />
            <span className={`text-sm font-medium ${isActive ? "text-emerald-600" : "text-slate-400"}`}>{isActive ? "활성" : "만료됨"}</span>
          </div>

          <div className="flex justify-end gap-2">
            <button onClick={() => setSeed(Math.floor(Math.random() * 0x7fffffff))}
              className="flex items-center gap-1.5 px-4 py-2 text-sm text-slate-500 border border-slate-200 rounded-lg hover:bg-slate-100 transition-colors">
              <RefreshCw size={13} /> 재발급
            </button>
            <button onClick={onClose} className="px-4 py-2 text-sm text-slate-900 bg-violet-600 rounded-lg hover:bg-violet-700 transition-colors">닫기</button>
          </div>
        </div>
      </div>
    </div>
