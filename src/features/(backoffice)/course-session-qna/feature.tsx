  ...
  if (visiblePosts.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-slate-400">
        <MessageCircle size={36} className="mb-3 text-slate-500" />
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className="flex items-center gap-2 px-1 pb-3">
        <span className="text-sm text-slate-500">총 {visiblePosts.length}개</span>
        {unansweredCount > 0 && (
        ...
        return (
          ...
          >
            ...
            <button
              ...
              className="w-full flex items-start gap-3 px-5 py-4 text-left hover:bg-slate-100/40 transition-colors"
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-slate-400 flex-shrink-0">{post.learnerName}</span>
            ...
            {/* 펼쳐진 내용 */}
            {expanded && (
              <div className="border-t border-slate-200 px-5 py-4 flex flex-col gap-4">
                {/* 질문 본문 */}
                ...
                {post.replies.map((reply) => (
                  <div key={reply.id} className="flex gap-3">
                    <div className="w-7 h-7 rounded-full bg-violet-500/20 flex items-center justify-center text-violet-600 text-xs font-semibold flex-shrink-0">
                      강
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                ...
                {post.replies.length === 0 && (
                  <div className="flex gap-2 pt-1">
                    <textarea
                      ...
                      rows={3}
                      className="flex-1 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-900 placeholder-slate-400 resize-none focus:outline-none focus:border-violet-500 transition-colors"
                    />
                    <button
                      onClick={() => handleReplySubmit(post.id)}
                      disabled={!draftBody[post.id]?.trim()}
                      className="self-end px-3 py-2 bg-violet-600 text-slate-900 rounded-lg hover:bg-violet-500 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                    >
