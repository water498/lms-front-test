...
const MEDIA_REQUIRED: ActivityType[] = ["VIDEO", "SCORM"];
...
export default function AddActivityModal({ subjectTitle, onClose }: Props) {
  ...
  const needsMedia = MEDIA_REQUIRED.includes(selectedType);
        ...
        {step === 1 && (
          <>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button onClick={handleNext} className="px-4 py-2 text-sm text-white bg-violet-600 rounded-lg hover:bg-violet-700 transition-colors">
                다음
              </button>
            </div>
          </>
                      ...
                      {filteredMedia.map((f) => (
                        ...
                        >
                          <input
                            ...
                            name="mediaFile"
                            value={f.id}
                            checked={selectedMediaId === f.id}
                            onChange={() => setSelectedMediaId(f.id)}
                            className="sr-only"
                          />
                          <div className={`w-2 h-2 rounded-full flex-shrink-0 border-2 ${
                            selectedMediaId === f.id ? "border-violet-600 bg-violet-600" : "border-slate-300"
                          }`} />
                          <div className="flex-1 min-w-0">
                            <p className={`text-sm truncate ${selectedMediaId === f.id ? "text-violet-700 font-medium" : "text-slate-700"}`}>
                              {f.name}
                            </p>
