...
export default function AddActivityModal({ subjectTitle, onAdd, onClose }: Props) {
  const [activeTab, setActiveTab]           = useState<ActivityTab>("MEDIA");
  const [title, setTitle]                   = useState("");
  const [mediaSearch, setMediaSearch]       = useState("");
  const [mediaTypeFilter, setMediaTypeFilter] = useState<"ALL" | "VIDEO" | "SCORM">("ALL");
  ...
  const filteredMedia = mediaAssets.filter((a) => {
    ...
    if (mediaTypeFilter === "ALL" && a.assetType !== "VIDEO" && a.assetType !== "SCORM") return false;
    if (mediaSearch && !a.displayName.toLowerCase().includes(mediaSearch.toLowerCase())) return false;
    return true;
  });
  ...
  function buildActivity(): Activity {
    ...
    if (activeTab === "MEDIA" && selectedMediaId) {
      ...
      const type: ActivityType = asset.assetType === "SCORM" ? "SCORM" : "VIDEO";
        ...
        {activeTab === "MEDIA" && (
          <div className="mb-5">
            ...
            {filteredMedia.length === 0 ? (
              <p className="text-xs text-slate-400 py-2">해당 유형의 활성 자료가 없습니다.</p>
            ) : (
                ...
                {filteredMedia.map((asset) => (
                  ...
                  >
                    <input
                      ...
                      className="sr-only"
                    />
                    <div className={`w-2 h-2 rounded-full flex-shrink-0 border-2 ${selectedMediaId === asset.id ? "border-violet-600 bg-violet-600" : "border-slate-300"}`} />
                    <span className="text-base flex-shrink-0">{asset.assetType === "SCORM" ? "📦" : "📹"}</span>
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm truncate ${selectedMediaId === asset.id ? "text-violet-700 font-medium" : "text-slate-700"}`}>
                        {asset.displayName}
                      </p>
                      <p className="text-xs text-slate-400">{asset.size} · {asset.assetType}</p>
                    </div>
                  </label>
                ))}
              </div>
            ...
            {examTemplates.length === 0 ? (
              ...
              <div className="flex flex-col gap-1.5 max-h-52 overflow-y-auto pr-1">
                {examTemplates.map((e) => (
                  ...
                  >
                    <input type="radio" name="examTemplate" value={e.id} checked={selectedExamId === e.id} onChange={() => selectExam(e.id, e.title)} className="sr-only" />
            ...
            {assignmentTemplates.length === 0 ? (
              ...
              <div className="flex flex-col gap-1.5 max-h-52 overflow-y-auto pr-1">
                {assignmentTemplates.map((a) => (
                  ...
                  >
                    <input type="radio" name="assignTemplate" value={a.id} checked={selectedAssignId === a.id} onChange={() => selectAssign(a.id, a.title)} className="sr-only" />
