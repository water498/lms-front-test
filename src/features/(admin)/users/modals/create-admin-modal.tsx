      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
          <div className="flex flex-col items-center gap-3 text-center">
            ...
            <h2 className="text-base font-semibold text-slate-800">관리자 계정이 생성되었습니다</h2>
            <p className="text-sm text-slate-500">
              <span className="font-medium text-slate-700">{name}</span>님의 계정이 생성되었습니다.<br />
              임시 비밀번호를 관리자에게 전달하세요.
            </p>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-base font-semibold text-slate-800">관리자 계정 생성</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600">
            <X size={18} />
          </button>
        </div>
        <p className="text-xs text-slate-500 mb-4">
          관리자는 별도 가입 없이 직접 생성합니다. 임시 비밀번호가 자동 발급됩니다.
        </p>
