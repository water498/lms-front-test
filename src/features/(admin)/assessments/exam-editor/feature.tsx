...
function newRule(): QuestionCompositionRule {
  return { id: makeId(), label: "", poolId: "", count: 5, shuffle: true };
}
...
// EXAM 그룹만 필터
const examPools = questionPools.filter((p) => p.kind === "EXAM" && !p.isArchived);
const poolMap = Object.fromEntries(examPools.map((p) => [p.id, p]));
...
export default function ExamEditorFeature({ examId }: Props) {
  ...
  const matchingQuestions = selected?.poolId
    ? bankQuestions.filter((q) => q.kind === "EXAM" && q.poolId === selected.poolId)
    : [];
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col gap-4">
                ...
                <div className="flex items-center gap-4">
                  <div className="flex-1">
                    ...
                    {selected.count > matchingQuestions.length && matchingQuestions.length > 0 && (
                      <p className="text-[11px] text-amber-500 mt-1">뱅크 문항({matchingQuestions.length})보다 출제 수가 많습니다</p>
                    )}
                  </div>
                  <div className="flex-1">
