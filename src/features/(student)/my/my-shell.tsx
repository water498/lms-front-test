...
const ALL_TABS = [
  { id: "learning",      label: "내 학습",    icon: <BookOpen className="w-4 h-4" />,  featureFlag: undefined },
  { id: "certificates",  label: "수료증",      icon: <Award className="w-4 h-4" />,     featureFlag: undefined },
  { id: "orders",        label: "주문 내역",   icon: <CreditCard className="w-4 h-4" />, featureFlag: "payments" as const },
  { id: "wishlist",      label: "위시리스트",  icon: <Heart className="w-4 h-4" />,     featureFlag: undefined },
  { id: "profile",       label: "내 정보",     icon: <Settings className="w-4 h-4" />,  featureFlag: undefined },
] as const;
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl w-full max-w-md p-6 flex flex-col gap-5">
        ...
        {submitted ? (
          <div className="flex flex-col items-center gap-3 py-6 text-center">
            ...
            <p className="text-sm font-semibold text-white">신청이 접수되었습니다!</p>
            <p className="text-xs text-zinc-400">
              검토 후 이메일로 결과를 안내드립니다.<br />보통 영업일 기준 3~5일 내 처리됩니다.
            </p>
            <button onClick={onClose} className="mt-2 px-4 py-2 text-sm bg-zinc-800 text-zinc-300 rounded-xl hover:bg-zinc-700 transition-colors">
              닫기
            </button>
          </div>
        ) : (
          <>
            <p className="text-sm text-zinc-400">강사로 활동하고 싶으신가요? 아래 정보를 작성해 주시면 검토 후 연락드립니다.</p>
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">전문 분야 <span className="text-violet-400">*</span></label>
                <input type="text" value={specialty} onChange={(e) => setSpecialty(e.target.value)} placeholder="예: React, 데이터 분석, 영어회화..."
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">강의 계획 <span className="text-violet-400">*</span></label>
                <textarea value={plan} onChange={(e) => setPlan(e.target.value)} rows={3} placeholder="어떤 강의를 만들고 싶으신지 간략하게 적어주세요."
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-zinc-400">관련 경력 (선택)</label>
                <textarea value={career} onChange={(e) => setCareer(e.target.value)} rows={2} placeholder="관련 직무 경험, 자격증, 포트폴리오 URL 등"
                  className="bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-violet-500 transition-colors resize-none" />
              </div>
            </div>
            <div className="flex gap-2 justify-end">
              <button onClick={onClose} className="px-4 py-2 text-sm text-zinc-400 hover:text-white transition-colors">취소</button>
              <button onClick={handleSubmit} disabled={!specialty.trim() || !plan.trim()}
...
export default function MyShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
        ...
        <div className="max-w-screen-xl mx-auto px-6 py-10 flex flex-col lg:flex-row gap-8 items-start">
          ...
          <aside className="w-full lg:w-64 shrink-0">
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 flex flex-col items-center gap-3 mb-4">
              <div className="w-16 h-16 rounded-full bg-violet-600 flex items-center justify-center text-xl font-bold text-white">홍</div>
      ...
      {showApplyModal && <InstructorApplyModal onClose={() => setShowApplyModal(false)} />}
    </>
  );
}
